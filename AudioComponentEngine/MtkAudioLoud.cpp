...
ACE_ERRID MtkAudioCustParamCache::loadParameter(uint32_t FilterType, AUDIO_ACF_CUSTOM_PARAM_STRUCT *AudioParam) {
    ...
    if (FilterType == mFilterType) {
        ...
        if (FilterType == AUDIO_COMP_FLT_AUDENH) {
            ...
            getAudioCompFltCustParamFromStorage((AudioCompFltType_t)FilterType, AudioParam);
        }
...
MtkAudioLoud::MtkAudioLoud() :
    ...
    mIsSepLR_Filter(false), mIsZeroCoeff(false), mNoiseFilter(0) {
    init();
}
...
#else
            getAudioCompFltCustParamFromStorage((AudioCompFltType_t)AUDIO_COMP_FLT_AUDIO_SUB, &mAudioParam);
#endif
            copyParamSub();
        }
#endif

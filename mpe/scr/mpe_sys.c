...
static MPE_THREAD_T MPE_thread = {C_INVALID_FD, MPE_THREAD_SOCKET, C_INVALID_TID, MPE_exit_thread_normal, NULL};
...
BL MPE_threads_create( void ) {
    ...
    if(pthread_create(&MPE_thread.thread_handle, NULL, thread_socket_MPE,NULL)) {
        ret = FALSE;
...
int MPE_threads_release(void) {
    ...
    if ((MPE_thread.thread_exit(&MPE_thread))) {
        result = MPE_ERROR;
...
/*****************************************************************************
 * FUNCTION
 *  MPE_sys_socket_recv_msg
 * DESCRIPTION
 *  Receive from MNLD/FLPD socket
 * PARAMETERS: socket id, address, socket buffer
 * RETURNS
 *  TRUE/FALSE (MPE receive failed/passed)
 *****************************************************************************/
int MPE_sys_socket_recv_msg(mtk_int32 sockfd, struct sockaddr_un sock_addr[MPE_SOCKET_CONN_NO], char sockbuf[128]) {
    ...
    recvlen = recvfrom(sockfd, &sockbuf[0], 128, 0, (struct sockaddr*)&(sock_addr[0]), &addrlen);
 ...
 *****************************************************************************/
mtk_int32 MPE_sys_scan_socket(int *sock_fd, int size) {
    fd_set readfds;
    int recvlen = 0;
    int max_sd = 0, ret=0, idx;

    FD_ZERO(&readfds);
    for(idx=0;idx<size;idx++) {
        //FLPD("sock_fd =%d\n",sock_fd[idx]);
        if(sock_fd[idx]>max_sd) {
            max_sd = sock_fd[idx];
        }

        if(sock_fd[idx] > 0) {
            FD_SET(sock_fd[idx], &readfds);
        }
    }
    ...
    if(ret == -1) {
        ...
        for(idx=0;idx<size;idx++) {
            if(FD_ISSET(sock_fd[idx], &readfds)) {
                //FLPD("scanned socket %d\n",sock_fd[idx]);
                return sock_fd[idx];
            }
        }
    }

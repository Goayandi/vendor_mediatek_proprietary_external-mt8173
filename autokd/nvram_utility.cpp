...
int sdio_read_nvram(unsigned char *ucNvRamData)
{
    ...
    if(sdio_nvram_fd.iFileDesc >= 0){
        ...
        if(rec_size != sizeof(ap_nvram_sdio_config_struct)){
            LOGE("Unexpected record size %d ap_nvram_sdioradio_struct %zu",
                    rec_size, sizeof(ap_nvram_sdio_config_struct));
            NVM_CloseFileDesc(sdio_nvram_fd);
            return -1;
        }
        
        readlen = read(sdio_nvram_fd.iFileDesc, &sdio_nvram, rec_num*rec_size);
        if(readlen < 0){
            LOGE("Read NVRAM fails errno %d\n", errno);
            NVM_CloseFileDesc(sdio_nvram_fd);
            return -1;
        } else if (readlen != rec_num*rec_size) {
            LOGE("read rec_num*rec_size fail!\n");
        }
...
int write_nvram(unsigned char* write_buf, int length, int id, int file_idx, int file_count)
{
    ...
    if(sdio_nvram_fd.iFileDesc >= 0){
        // Write to data
        if( lseek(sdio_nvram_fd.iFileDesc, 1+MAX_COUNT+(MAX_COUNT*sizeof(unsigned int))+(file_idx*EACH_FILE_SIZE), SEEK_SET) == -1) {
            LOGE("lseek fail! errno = %d\n",errno);
        }
        ...
        if( lseek(sdio_nvram_fd.iFileDesc, 1+MAX_COUNT+(file_idx*sizeof(unsigned int)), SEEK_SET) == -1) {
            LOGE("lseek fail! errno = %d\n",errno);
        }
        ...
        // Write to host_id
        if( lseek(sdio_nvram_fd.iFileDesc, 1+file_idx, SEEK_SET) == -1) {
            LOGE("lseek fail! errno = %d\n",errno);
        }
        ...
        // Write to file_count
        if( lseek(sdio_nvram_fd.iFileDesc, 0, SEEK_SET) == -1) {
            LOGE("lseek fail! errno = %d\n",errno);
        }
...
int write_file_to_nvram(char *filename, int id)
{
    ...
    if (fread (data_buf, 1, lSize, inFile) != lSize) {
        LOGE("fread() != lSize \n");
    }

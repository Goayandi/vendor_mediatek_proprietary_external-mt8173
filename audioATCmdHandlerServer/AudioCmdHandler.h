...
typedef enum{
	AUDIO_ACF_FO       = 0x212,
    AUDIO_GET_LGE_DISP = 0x500,
    AUDIO_GET_BT_NREC_DISP = 0x501,
    AUDIO_DATA_TEST = 0x502,
}AudioCommands;
...
class RecorderListener : public MediaRecorderListener
{
    public:
        RecorderListener(AudioCmdHandler *pListener);
        ~RecorderListener() {}
...
class AudioCmdHandler : public MediaPlayerListener
{
...
#if 0
        ACHStatus ULCustACFParamFromNV(char *pParam, int *len);
        ACHStatus DLCustACFParamToNV(char *pParam);
#else
...
#endif
	ACHStatus findACFFO(unsigned short cmdType, char *pParam, int *len);
#if 0
        ACHStatus ULCustHCFParamFromNV(char *pParam, int *len);
        ACHStatus DLCustHCFParamToNV(char *pParam);
#endif
	...
	bool GainInfoFlag;
        ...
        sp <RecorderListener> m_pMediaRecorderListenner;
        ...
        status_t GetAudioCommand(int par1, int *par2);

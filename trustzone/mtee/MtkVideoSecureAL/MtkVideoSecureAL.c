...
unsigned long MtkVideoAllocateSecureBuffer(int size, int align, const char* tag) {
    ...
    // Alloc secure memory
    ret = UREE_AllocSecurememWithTag (gMemSession, &memHandle, align, size, tag);
    if (ret != TZ_RESULT_SUCCESS)
    {
...
/* Begin added by mtk09845 UT_TEST_DEV  */
unsigned long MtkVideoAllocateSecureBitstreamBuffer(int size, int align, const char *tag)
{
    ...
    ret = UREE_AllocSecurechunkmemWithTag(gMemSession, &memHandle, align, size, tag);
    if (ret != TZ_RESULT_SUCCESS)
    {
...
unsigned long MtkVdecFillSecureBuffer(uint32_t bs_sec_handle, void *bs_va, uint32_t size, uint32_t bdirection)
{
    ...
    pBitstream_va_share_handle = MtkVideoRegisterSharedMemory(bs_va, size, "MtkVdecFillSecureBuffer");
    ...
    pFillStuct_share_handle = MtkVideoRegisterSharedMemory(pFillStruct, sizeof(VdecFillSecMemStruct), "MtkVdecFillSecureBuffer");
                                       ...
                                       unsigned int* pH265_Sec_session)
{
    ...
    pVdec_H265_Drv_data_share_handle        = MtkVideoRegisterSharedMemory(pInitStruct[0].mem, pInitStruct[0].size, "MtkVdecH265SecInit");
    pVdec_H265_InstanceTemp_share_handle    = MtkVideoRegisterSharedMemory(pInitStruct[1].mem, pInitStruct[1].size, "MtkVdecH265SecInit");
    pH265_DEC_PRM_DataInst_share_handle     = MtkVideoRegisterSharedMemory(pInitStruct[2].mem, pInitStruct[2].size, "MtkVdecH265SecInit");
    pDecStruct2_share_handle                = MtkVideoRegisterSharedMemory(pInitStruct[4].mem, pInitStruct[4].size, "MtkVdecH265SecInit");
    ...
    *DecStruct2_share_handle = pDecStruct2_share_handle;
    ...
    UREE_SHAREDMEM_HANDLE pVdecH265SecInit_share_handle =
                        MtkVideoRegisterSharedMemory(ph265_sec_init_struct, sizeof(VdecH265SecInitStruct), "MtkVdecH265SecInit");
    #if 0
...
unsigned long MtkVideoRegisterSharedMemory(void* buffer, int size, const char *tag) {
    TZ_RESULT ret;
    ...
    ret = UREE_RegisterSharedmemWithTag(gMemSession, &sharedHandle, &sharedParam, tag);
    if (ret != TZ_RESULT_SUCCESS)
    {
        ALOGE ("UREE_RegisterSharedmemWithTag Error: %d, line:%d, gMemSession(%d)", ret, __LINE__, gMemSession);
    }
    else
    {
       ALOGI("UREE_RegisterSharedmemWithTag: %d, line:%d, gMemSession(%d)", ret, __LINE__, gMemSession);
    }
    return sharedHandle;
}
...
MTK_SECURE_AL_RESULT MtkVideoUnregisterSharedMemory(unsigned long sharedHandle)
{
    ...
    if (ret != TZ_RESULT_SUCCESS)
    {
        ...
        ALOGI("UREE_UnregisterSharedmem: %d, line:%d, gMemSession(%d),sharedHandle(%d)", ret, __LINE__, gMemSession,sharedHandle);
    }
...
MTK_SECURE_AL_RESULT MtkVdecH264SecInitTest(VdecFrameDumpStruct* pFrameDumpInfo) {
    ...
    pVdecFrameDump_share_handle = MtkVideoRegisterSharedMemory(pFrameDumpInfo, sizeof(VdecFrameDumpStruct), "MtkVdecH264SecInitTest");
...
MTK_SECURE_AL_RESULT MtkVdecH264SecInit(VdecH264SecMemInfoStruct* pInitStruct, unsigned long memHandle,
    ...
    pVdec_H264_Drv_data_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[0].mem, pInitStruct[0].size, "MtkVdecH264SecInit");
    pVdec_H264_InstanceTemp_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[1].mem, pInitStruct[1].size, "MtkVdecH264SecInit");
    pH264_DEC_PRM_DataInst_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[2].mem, pInitStruct[2].size, "MtkVdecH264SecInit");
    pDecStruct2_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[4].mem, pInitStruct[4].size, "MtkVdecH264SecInit");
    ...
    *DecStruct2_share_handle = pDecStruct2_share_handle;
    ...
    //ALOGE ("@@ [0x%08X][0x%08X][0x%08X]", pVdec_H264_InstanceTemp_share_handle, pH264_DEC_PRM_DataInst_share_handle, pVdec_H264_Drv_data_share_handle);
     ...
     pBitstream_va_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[3].mem, pInitStruct[3].size, "MtkVdecH264SecInit");
     }
     else {
    ...
    pVdecH264SecInit_share_handle = MtkVideoRegisterSharedMemory(ph264_sec_init_struct, sizeof(VdecH264SecInitStruct), "MtkVdecH264SecInit");
    param[0].memref.handle = (uint32_t) pVdecH264SecInit_share_handle;
    param[0].memref.offset = 0;
    param[0].memref.size = sizeof(VdecH264SecInitStruct);
...
*/
    ...
    if(secAllocSize> 0)
    {
        ret = UREE_AllocSecurechunkmemWithTag (gMemSession, &gReservedMem, 1024, secAllocSize, "MtkVdecH264SecInit");
        if (ret != TZ_RESULT_SUCCESS)
        {
            ALOGE ("UREE_AllocSecurechunkmemWithTag Error: %d, line:%d\n", ret, __LINE__);
        }
...
MTK_SECURE_AL_RESULT MtkVdecH264SecDeinit(unsigned int H264_Sec_session, VdecH264SecDeinitStruct* pDeinitStruct) {
    ...
    if(gReservedMem > 0)
    {
        ret = UREE_UnreferenceSecurechunkmem (gMemSession, gReservedMem);
        if (ret != TZ_RESULT_SUCCESS)
        {
            ALOGE ("UREE_UnreferenceSecurechunkmem Error: %d, line:%d", ret, __LINE__);
        }
...
#endif
    pDeinitStruct_share_handle = MtkVideoRegisterSharedMemory(pDeinitStruct, sizeof(VdecH264SecDeinitStruct), "MtkVdecH264SecDeinit");
...
MTK_SECURE_AL_RESULT MtkVdecH265SecDeinit(unsigned int H265_Sec_session, VdecH265SecDeinitStruct* pDeinitStruct) {
    ...
    pDeinitStruct_share_handle = MtkVideoRegisterSharedMemory(pDeinitStruct, sizeof(VdecH265SecDeinitStruct), "MtkVdecH265SecDeinit");
...
MTK_SECURE_AL_RESULT MtkVdecH264SecDecode(unsigned int H264_Sec_session, VdecH264SecMemInfoStruct* pDecStruct1, unsigned long pDecStruct2_share_handle) {
    ...
    if (pDecStruct1[0].mem != NULL) {
        pBitstream_va_share_handle = MtkVideoRegisterSharedMemory(pDecStruct1[0].mem, pDecStruct1[0].size, "MtkVencH264SecDecode");
...
MTK_SECURE_AL_RESULT MtkVdecVP9SecInit(VdecVP9SecMemInfoStruct* pInitStruct,
    ...
    pVdec_VP9_Drv_data_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[0].mem, pInitStruct[0].size,"VP9_Drv_data_share_handle");
    pDecStruct2_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[2].mem, pInitStruct[2].size,"VP9_pDecStruct2_share_handle");
    pVdec_prCommon_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[3].mem, pInitStruct[3].size,"VP9_prCommon_share_handle");
    ...
    *prCommon_share_handle = pVdec_prCommon_share_handle;
    ...
    if (pInitStruct[1].mem != NULL) {
      pBitstream_va_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[1].mem, pInitStruct[1].size,"VP9_pBitstream_va_share_handle");
    }
    else {
      pBitstream_va_share_handle = 0;
    }
    ...
    pVdecVP9SecInit_share_handle = MtkVideoRegisterSharedMemory(pvp9_sec_init_struct, sizeof(VdecVP9SecInitStruct),"VP9SecInit_share_handle");
...
#if UNREGISTER_SHARE_MEM
    if (MTK_SECURE_AL_SUCCESS != MtkVideoUnregisterSharedMemory(pVdecVP9SecInit_share_handle)) {
      ALOGE ("MtkVideoUnregisterSharedMemory failed, line:%d", __LINE__);
      free(pvp9_sec_init_struct);
      return MTK_SECURE_AL_FAIL;
    }
...
MTK_SECURE_AL_RESULT MtkVdecVP9SecDeinit(unsigned int VP9_Sec_session,VdecVP9SecDeinitStruct* pDeinitStruct)
{
    ...
    pDeinitStruct_share_handle = MtkVideoRegisterSharedMemory(pDeinitStruct, sizeof(VdecVP9SecDeinitStruct),"VP9_pDeinitStruct_share_handle");
    ...
    if (MTK_SECURE_AL_SUCCESS != MtkVideoUnregisterSharedMemory(pDeinitStruct->pVdec_VP9_Drv_data_share_handle)) {
        ALOGE ("MtkVideoUnregisterSharedMemory failed, line:%d", __LINE__);
        return MTK_SECURE_AL_FAIL;
    }

    if (MTK_SECURE_AL_SUCCESS != MtkVideoUnregisterSharedMemory(pDeinitStruct->pVdec_prcommon_share_handle)) {
        ALOGE ("MtkVideoUnregisterSharedMemory failed, line:%d", __LINE__);
        return MTK_SECURE_AL_FAIL;
    }
    if (MTK_SECURE_AL_SUCCESS != MtkVideoUnregisterSharedMemory(pDeinitStruct->pDecStruct2_share_handle)) {
...
#if UT_ENABLE   // share ring buffer VA to TEE
    if (pDecStruct1[0].mem != NULL) {
        pBitstream_va_share_handle = MtkVideoRegisterSharedMemory(pDecStruct1[0].mem, pDecStruct1[0].size,"pBitstream_va_share_handle");
    }else {
        pBitstream_va_share_handle = 0;
    }
#endif
...
MTK_SECURE_AL_RESULT MtkVencH264SecInit(VideoH264SecMemInfoStruct* pInitStruct, VAL_UINT32_T * H264_Drv_data_share_handle)
{
    ...
    pVenc_H264_Drv_data_share_handle = MtkVideoRegisterSharedMemory(pInitStruct[0].mem, pInitStruct[0].size, "MtkVencH264SecInit");
    ...
    *H264_Drv_data_share_handle = pVenc_H264_Drv_data_share_handle;
    ...
    pVencH264SecInit_share_handle = MtkVideoRegisterSharedMemory(ph264_sec_init_struct, sizeof(VencH264SecInitStruct), "MtkVencH264SecInit");
...
MTK_SECURE_AL_RESULT MtkVencH264SecEncode(VAL_UINT32_T H264_Drv_data_share_handle, VideoH264SecMemInfoStruct* pEncStruct, VideoH264SecMemInfoStruct* pParStruct, unsigned long EncodeOption, unsigned char fgSecBuffer)
{
    ...
    if(pEncStruct[0].mem)
    {
        pVenc_H264_Frame_share_handle = MtkVideoRegisterSharedMemory(pEncStruct[0].mem, pEncStruct[0].size, "MtkVencH264SecEncode");  //Frame Buffer Structure
        ALOGI ("+MtkVencH264SecEncode pVenc_H264_Frame_share_handle[0x%x]\n",pVenc_H264_Frame_share_handle);
    }
    pVenc_H264_Bitstream_share_handle = MtkVideoRegisterSharedMemory(pEncStruct[1].mem, pEncStruct[1].size, "MtkVencH264SecEncode");    //Bitstream Buffer Structure
    ALOGI ("+MtkVencH264SecEncode pVenc_H264_Bitstream_share_handle[0x%x]\n",pVenc_H264_Bitstream_share_handle);
    pVenc_H264_Encresult_share_handle = MtkVideoRegisterSharedMemory(pEncStruct[2].mem, pEncStruct[2].size, "MtkVencH264SecEncode");  //EncResult Structure
    ...
    if(fgSecBuffer)
    {
         ...
         if(pParStruct[0].mem)
         {
           ...
           pVenc_H264_FrameBuf_share_handle = MtkVideoRegisterSharedMemory(pParStruct[0].mem, pParStruct[0].size, "MtkVencH264SecEncode"); //Real Bitstream Buffer VA
...
#ifdef BIN_SEC_SELF_TEST  //Only for BIN_SELF_TEST
        pVenc_H264_BitstreamBuf_share_handle = MtkVideoRegisterSharedMemory(pParStruct[1].mem, pParStruct[1].size, "MtkVencH264SecEncode"); //Real Bitstream Buffer VA
    ...
    else
    {
        ...
        if(pParStruct[0].mem)
        {
            pVenc_H264_FrameBuf_share_handle = MtkVideoRegisterSharedMemory(pParStruct[0].mem, pParStruct[0].size, "MtkVencH264SecEncode"); //Real Frame Buffer VA
            ALOGI ("+MtkVencH264SecEncode pVenc_H264_FrameBuf_share_handle[0x%x]\n",pVenc_H264_FrameBuf_share_handle);
        }
        pVenc_H264_BitstreamBuf_share_handle = MtkVideoRegisterSharedMemory(pParStruct[1].mem, pParStruct[1].size, "MtkVencH264SecEncode"); //Real Bitstream Buffer VA
    ...
    pVencH264SecEnc_share_handle = MtkVideoRegisterSharedMemory(ph264_sec_enc_struct, sizeof(VencH264SecEncStruct), "MtkVencH264SecEncode");
...
MTK_SECURE_AL_RESULT MtkVencH264SecAllocWorkBuf(VAL_UINT32_T H264_Drv_data_share_handle)
{
    ...
    pVencH264SecCmd_share_handle = MtkVideoRegisterSharedMemory(ph264_sec_cmd_struct, sizeof(VencH264SecCmdStruct), "MtkVencH264SecAllocWorkBuf");
...
MTK_SECURE_AL_RESULT MtkVencH264SecFreeWorkBuf(VAL_UINT32_T H264_Drv_data_share_handle)
{
    ...
    ph264_sec_cmd_struct->venc_h264_drv_data_share_handle = pVenc_H264_Drv_data_share_handle;
    pVencH264SecCmd_share_handle = MtkVideoRegisterSharedMemory(ph264_sec_cmd_struct, sizeof(VencH264SecCmdStruct),"MtkVencH264SecFreeWorkBuf");
    param[0].memref.handle = (uint32_t) pVencH264SecCmd_share_handle;
    param[0].memref.offset = 0;
    param[0].memref.size = sizeof(VencH264SecCmdStruct);
...
MTK_SECURE_AL_RESULT MtkVencH264SecShareWorkBuf(VAL_UINT32_T H264_Drv_data_share_handle, VideoH264SecMemInfoStruct* pCmdStruct, unsigned long* pShareHandle)
{
    ...
    pVenc_H264_RCInfo_share_handle = MtkVideoRegisterSharedMemory(pCmdStruct[0].mem, pCmdStruct[0].size, "MtkVencH264SecShareWorkBuf");
    pVenc_H264_RCCode_share_handle = MtkVideoRegisterSharedMemory(pCmdStruct[1].mem, pCmdStruct[1].size, "MtkVencH264SecShareWorkBuf");
    pVenc_H264_RecLuma_share_handle = MtkVideoRegisterSharedMemory(pCmdStruct[2].mem, pCmdStruct[2].size, "MtkVencH264SecShareWorkBuf");
    pVenc_H264_RecChroma_share_handle = MtkVideoRegisterSharedMemory(pCmdStruct[3].mem, pCmdStruct[3].size, "MtkVencH264SecShareWorkBuf");
    pVenc_H264_RefLuma_share_handle = MtkVideoRegisterSharedMemory(pCmdStruct[4].mem, pCmdStruct[4].size, "MtkVencH264SecShareWorkBuf");
    pVenc_H264_RefChroma_share_handle = MtkVideoRegisterSharedMemory(pCmdStruct[5].mem, pCmdStruct[5].size, "MtkVencH264SecShareWorkBuf");
    pVenc_H264_MVInfo1_handle = MtkVideoRegisterSharedMemory(pCmdStruct[6].mem, pCmdStruct[6].size, "MtkVencH264SecShareWorkBuf");
    pVenc_H264_MVInfo2_handle = MtkVideoRegisterSharedMemory(pCmdStruct[7].mem, pCmdStruct[7].size, "MtkVencH264SecShareWorkBuf");
    pVenc_H264_PPSTemp_share_handle = MtkVideoRegisterSharedMemory(pCmdStruct[8].mem, pCmdStruct[8].size, "MtkVencH264SecShareWorkBuf");
    pVenc_H264_IDRTemp_share_handle = MtkVideoRegisterSharedMemory(pCmdStruct[9].mem, pCmdStruct[9].size, "MtkVencH264SecShareWorkBuf");
    ...
    pVencH264SecCmd_share_handle = MtkVideoRegisterSharedMemory(ph264_sec_cmd_struct, sizeof(VencH264SecShareWorbufStruct), "MtkVencH264SecShareWorkBuf");
    param[0].memref.handle = (uint32_t) pVencH264SecCmd_share_handle;
    param[0].memref.offset = 0;
...
MTK_SECURE_AL_RESULT MtkVencH264SecCopyWorkBuf(VAL_UINT32_T H264_Drv_data_share_handle)
{
    ...
    ph264_sec_cmd_struct->venc_h264_drv_data_share_handle = pVenc_H264_Drv_data_share_handle;
    pVencH264SecCmd_share_handle = MtkVideoRegisterSharedMemory(ph264_sec_cmd_struct, sizeof(VencH264SecCmdStruct), "MtkVencH264SecCopyWorkBuf");
    param[0].memref.handle = (uint32_t) pVencH264SecCmd_share_handle;
    param[0].memref.offset = 0;
    param[0].memref.size = sizeof(VencH264SecCmdStruct);

...
static const int mDsmBufferSize = 1024 * 32;
...
static void AssignedDSMParameters(const mlds_task_config_t *p_mlds_task_config)
{
    ...
    if(p_mlds_task_config->stream_downlink.sample_rate_in == 16000)
    {
        mDsmSamples = MAXIM_SPEAKER_VOICE_PROCESSING_SAMPLE;
    ...
    {
        mDsmSamples = MAXIM_SPEAKER_PROCESSING_SAMPLE;
        mDsmIVProcessingSize = mDsmSamples * p_mlds_task_config->stream_downlink.num_channels_out * 2 ;
        mDsmProcessingSize = mDsmSamples * p_mlds_task_config->stream_downlink.num_channels_out;
    }
    ...
    void **pp_handler)
{
        ...
        p_mlds_task_config->stream_downlink.bit_format_in == BIT_FORMAT_S32_LE)
    {
        ...
        #endif
        ALOGD("dsm_handler = %p DSM_API_Init mDsmWorkingChannel = %d sample_rate_in = %d iSampleBitWidth= %d",
            dsm_handler,mDsmWorkingChannel,p_mlds_task_config->stream_downlink.sample_rate_in,sInitParam.iSampleBitWidth );

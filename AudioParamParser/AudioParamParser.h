...
#define XML_CUS_STORAGE_PATH            "/sdcard/"
...
typedef struct AppOps{
    ...
    char            *(*utilNativeGetChecklist)(const char *audioTypeName, const char *paramName, const char *fieldName);
...
{
#ifdef WIN32
    ALOGW("Donot support appOpsGetInstance for WIN32");
    return NULL;
#else
    const char *error;
    ...
    if (appOpsInited == 0)
    {
        ALOGD("%s(), init AppOps struct", __FUNCTION__);

        /* dlopen */
        appOps.handle = dlopen("libaudio_param_parser.so", RTLD_LAZY);
        if (!appOps.handle) {
        ...
        appOps.utilNativeGetChecklist = (char* (*)(const char *audioTypeName, const char *paramName, const char *fieldName)) dlsym(appOps.handle, funName);
        error = dlerror();
        if (error != NULL) {
            ALOGE("%s(), dlsym %s fail. (%s)\n", __FUNCTION__, funName, error);
            return NULL;

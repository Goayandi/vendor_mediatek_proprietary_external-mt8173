...
EXPORT APP_STATUS appHandleLoadDirFeatureOptionsInfo(AppHandle *appHandle)
{
    ...
    for (i = 0; appHandle->xmlDir[i]; i++)
    {
        ...
        sprintf(featureOptionsFile, "%s%s%s", appHandle->xmlDir[i], FOLDER, FEATURE_OPTIONS_XML);
...
#if 0
    /* Checking if the cust XML dir ready */
    if (access(XML_CUS_STORAGE_PATH, F_OK) == 0)
    {
...
EXPORT char *appHandleGetAudioTypeFilePath(AppHandle *appHandle, const char *audioType, const char *posfix)
{
    ...
    if (appHandle->xmlChangedNotifyEnabled && appHandle->xmlCusDir && !strcmp(posfix, AUDIO_PARAM_XML_POSFIX))
    {
    ...
    for (i = 0; appHandle->xmlDir[i]; i++)
    {
        ...
        path = (char *)malloc(strLen);
        sprintf(path, "%s%s%s%s", appHandle->xmlDir[i], FOLDER, audioType, posfix);
...
EXPORT void appHandleReloadCustXml(AppHandle *appHandle)
{
    ...
    else
    {
        while (n--)
        {
            ...
            INFO_LOG("Notify xml file changed. (%s)\n", audioType->name);
...
EXPORT void *appHandleThreadLoop(void *arg)
{
    ...
    while (1)
    {
        if (inotify_add_watch(appHandle->inotifyFd, appHandle->xmlCusDir, IN_CLOSE_WRITE | IN_DELETE) < 0)
        {
            INFO_LOG("inotify_add_watch failed! sleep 1 sec to wait sdcard ready...");
            utilMkdir(appHandle->xmlCusDir);
            utilUsleep(1000000);
        }
        else
...
EXPORT APP_STATUS appHandleGetAudioTypeSupportedVerInfo(const char *audioTypeName, int *paramUnitDescVerMaj, int *paramUnitDescVerMin, int *audioParamVerMaj, int *audioParamVerMin)
{
    ...
    *audioParamVerMin = 0;
    ...
    WARN_LOG("%s AudioType version support info not found! Set the version with default 1.0\n", audioTypeName);
    return APP_NO_ERROR;
}

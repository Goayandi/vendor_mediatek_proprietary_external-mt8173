    ...
    switch (dataType)
    {
        ...
        case TYPE_BYTE_ARRAY:
        {
            ...
            else
            {
                ...
                char* restOfStr = NULL;
                elemData = strtok_r(tmpStr, ARRAY_SEPERATOR, &restOfStr);
                ...
                else
                {
                    ...
                    while ((elemData = strtok_r(NULL, ARRAY_SEPERATOR, &restOfStr)))
                    {
                        convVal = strtol(elemData, NULL, 0);
                        if (errno == ERANGE)
                        {
...
EXPORT UT_string *utilNormalizeCategoryGroupPathForAudioType(const char *categoryPath, AudioType *audioType)
{
    ...
    if ((categoryType = strtok_r(tmpCategoryPath, ARRAY_SEPERATOR, &restOfStr)) == NULL)
    {
        free(tmpCategoryPath);
        return searchPath;
    }

    if ((category = strtok_r(NULL, ARRAY_SEPERATOR, &restOfStr)) == NULL)
    {
        free(tmpCategoryPath);
        return searchPath;
    }
    ...
    while ((categoryType = strtok_r(NULL, ARRAY_SEPERATOR, &restOfStr)))
    {
        if ((category = strtok_r(NULL, ARRAY_SEPERATOR, &restOfStr)))
        {
            pair = strPairCreate(categoryType, category);
            HASH_ADD_KEYPTR(hh, strPairHash, pair->key, strlen(pair->key), pair);
        }
